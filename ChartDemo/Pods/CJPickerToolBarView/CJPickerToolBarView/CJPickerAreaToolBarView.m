...
#import "CJPickerAreaToolBarView.h"

@implementation CJPickerAreaToolBarView
...
- (id)initWithNibNameDefaultAndDelegate:(id<CJPickerAreaToolBarViewDelegate>)delegate{
    self = [[CJPickerAreaToolBarView alloc]initWithNibName:@"CJPickerAreaToolBarView" delegate:delegate];
    return self;
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    switch (component) {
        ...
        case 2:
        {
            ...
            NSString *string2 = @"";
            if ([datasC count] > 0) {
                string2 = [datasC objectAtIndex:0];
            }
            //NSLog(@"%@", string2);
            [self.selecteds replaceObjectAtIndex:2 withObject:string2];
